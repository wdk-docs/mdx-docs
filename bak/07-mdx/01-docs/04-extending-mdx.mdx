...
本文解释了如何扩展MDX内容，特别是如何使用插件转换内容。 {/* more */}
关于如何传递组件、道具和布局，请参见[§使用MDX][use]。
有关如何将MDX集成到您的项目中，请参阅[§入门][start]。
...
- 挂钩到编译的几个阶段的插件 (请参阅[remark插件][remark-plugins], [rehype 插件][rehype-plugins], 以及新的recma插件)
- 在运行时传递、定义或导入的组件 (see [§ Using MDX][use])

大多数时候，这些组件和插件都没有耦合到MDX。
例如，如果您正在使用React，那么您可以将[`<ReactConfetti />`][react-confetti]与MDX一起使用。
或者，您可以使用插件[`remark-gfm`][remark-gfm]打开MDX中的gfm功能。
...
- [`PaulieScanlon/mdx-embed`](https://github.com/PaulieScanlon/mdx-embed)
  — React组件用于嵌入第三方内容，与MDX提供商集成
- [`system-ui/theme-ui`](https://github.com/system-ui/theme-ui)
  — React组件用于构建一致的应用程序，与MDX提供商集成
...
<Note type="info">
  是否有另一个*专门*适用于MDX的组件？请发送PR添加到此处！
</Note>

### 插件列表

另请参阅[remark插件列表][remark-plugins]和[rehype插件列表][rehype-plugins].
...
- [`remcohaszing/recma-nextjs-static-props`](https://github.com/remcohaszing/recma-nextjs-static-props)
  — 生成[`getStaticProps`](https://nextjs.org/docs/basic-features/data-fetching/get-static-props)，在Next.js中公开顶级标识符
...
<Note type="info">
  是否有另一个*专门*与MDX配合使用的统一插件？请发送PR添加到此处！
</Note>

## 使用插件

插件的名称编码在传递插件的位置: 
remark插件进入`remarkPlugins`和在[`ProcessorOptions`][processor-options]的`reypePlugins`中重新键入插件.
...
为MDX创建插件与为remark或retype创建插件基本相同。
在[§在`unifiedjs.com`上学习][learn]中有几个关于这方面的指南和食谱.

对于插件的MDX特定部分，建议阅读[¶Architecture][architecture]以了解`@MDX-js/MDX`的工作原理。
有关表示MDX特定功能的节点类型的信息，请参阅`remark MDX`中的¶语法树][syntax-tree]，并使用我们的交互式[§游乐场][playground].
