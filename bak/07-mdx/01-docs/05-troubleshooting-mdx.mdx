...
本文介绍了使用MDX时可能出现的几个常见问题和错误。 {/* more */}
要了解MDX格式是如何工作的，我们建议您从[§什么是MDX][what]开始.
如何使用[我们的软件包](/packages/)在它们的自述文件中有说明。
要迁移到最新的MDX，请参阅[§从v1迁移到v2][migation-v2].
...
它们自2015年以来一直在开发中，我们从一开始就在MDX文件中支持它们，在MDX版本2中，我们完全改用它们。
许多工具已经支持ESM。
大多数其他工具都在努力支持它们。
有些仍然需要额外的配置。

有一个很棒的[Gist by **@sindresorhus**](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)详细解释了如何将ESM与许多不同的工具一起使用。
如果您在使用MDX、其他一些工具和ESM时遇到问题，您很可能会在那里找到您想要的东西。
请仔细阅读。
如果您仍然有问题，您正在集成MDX的工具的问题跟踪器可能会提供答案。

如果您在使用不支持ESM的工具（如Electron）时遇到问题，一个短期解决方案是使用捆绑器制作CJS版本。

<details>
  <summary>展开制作CJS捆绑包的示例</summary>
