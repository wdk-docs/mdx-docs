...
- 输出是序列化的JavaScript，仍需评估
- 注入注释以配置如何处理JSX
- 这是一个包含import/exports的完整文件
- 导出组件（`MDXContent`）
...
在[§什么是MDX][what]中，我们展示了大括号内的JavaScript表达式可以在MDX中使用:
...
- 用标记符写的东西的HTML等价物，如`h1`表示`# heading` (有关示例，请参见[§组件表][toc])
...
但是，当您嵌套MDX文件（将它们相互导入）时，可能会变得很麻烦。
就像这样:
...
如果不嵌套MDX文件，或者不经常嵌套它们，请不要使用提供程序：显式传递组件。
