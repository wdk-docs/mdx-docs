...
Node.js挂钩（也称为加载器）用于MDX。


## 这是什么？

此包包含Node.js挂钩，用于添加对导入MDX文件的支持。
Node *定制挂钩* 目前是一个候选版本。

## 我什么时候该用这个？

如果您正在使用Node并希望从文件系统导入MDX文件，则此集成非常有用。
...
> **Note**: 如果使用Node 20及更高版本，则会收到以下警告:
>
> ```txt
...
加载 `file:` 指向MD(X)文件的URL。

有关详细信息，请参见Node.js文档中的[`load`][node-load]。

### `Options`

配置（TypeScript类型）。

选项与[`@mdx-js/mdx`中的`CompileOptions`][compile-options]相同，不同之处在于`development`选项是根据您配置webpack的方式支持的。
您不能手动传递它。
...
这意味着我们尽量保持当前的发布线, `@mdx-js/node-loader@^3`, 与Node.js 16兼容。
