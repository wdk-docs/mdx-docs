export const info = {
  author: [{ github: "wooorm", name: "Titus Wormer", twitter: "wooorm" }],
...
<Note type="legacy">
  **Note**: This is an old migration guide. See [§ Migrating from v2 to v3](/migrating/v3/). The below is kept as is for
  historical purposes.
</Note>
...
- `create-mdx`
  ...
  integrations you prefer and _then_ add MDX to it
...
```mdx chrome=no
<hgroup># This is a heading now</hgroup>
```
...
````mdx chrome=no
<article>
  <hgroup># This is a heading now, not code or plain text</hgroup>
  <section>```js // if you do want code blocks, use fenced code ```</section>
</article>
````
...
You can also use them as an escape hatch if you ever _just_ want strings or JSX
...
{" "}
<h1>This just JSX, these *asterisks* have no meaning.</h1>}
